    ...
    try:
        ...
        if analyzed_vulns:
            ...
            for v in analyzed_vulns:
                if v.file_path and v.file_path not in source_code_map:
                    try:
                        # 防御性路径校验：防止路径穿越（即使 file_path 来自数据库）
                        file_real = os.path.realpath(v.file_path)
                        if proj_path:
                            proj_real = os.path.realpath(proj_path)
                            if not file_real.startswith(proj_real + os.sep) and file_real != proj_real:
                                source_code_map[v.file_path] = v.source_code or ""
                                continue
                        with open(file_real, encoding='utf-8', errors='ignore') as f:
                            source_code_map[v.file_path] = f.read()
                    except Exception:
                        source_code_map[v.file_path] = v.source_code or ""
